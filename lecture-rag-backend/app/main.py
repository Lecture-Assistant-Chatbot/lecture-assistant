...
class QueryRequest(BaseModel):
    prompt: str
    history: List[HistoryItem] = []  # optional history
...
async def get_query_embedding(text: str) -> List[float]:
    ...
    if not isinstance(vector, list) or not vector:
        raise RuntimeError(
            "Embedding API response did not include an embedding vector."
        )
...
# ---------- Google access token (optional helper) ----------
...
async def vertex_ai_vector_search_texts(
    query_vector: List[float], neighbor_count: int = 4
) -> list[str]:
    ...
    if not index_endpoint.startswith("projects/"):
        index_endpoint_name = (
            f"projects/{project_id}/locations/{location}/indexEndpoints/{index_endpoint}"
        )
    else:
    ...
    def _find_neighbors_sync():
        ...
        me_endpoint = aiplatform.MatchingEngineIndexEndpoint(
...
def construct_gemini_payload(
    query: str, context: str, history: List[HistoryItem]
) -> dict:
    system_instruction = (
        ...
        "Use the conversation history only when it is clearly relevant to the "
        "current question. "
...
app = FastAPI(
    ...
    description=(
        "Backend API for Retrieval-Augmented Generation using Vertex AI "
        "Vector Search and Gemini models."
    ),
    version="1.0.0",
)
